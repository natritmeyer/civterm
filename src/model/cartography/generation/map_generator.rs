...
impl MapGenerator {
    ...
    pub fn set_to_ocean(&self, map: &mut Map) {
        for y in 0..map.height {
            for x in 0..map.width {
                map.tile_at_mut(Location::new(x as u16, y as u16)).geography = Geography::Ocean;
            }
        }
    }
    ...
    pub fn set_border_tundra(&self, map: &mut Map) {
        for x in 0..map.width {
            map.tile_at_mut(Location::new(x as u16, 0)).geography = Geography::Tundra;
            map.tile_at_mut(Location::new(x as u16, (map.height - 1) as u16))
                .geography = Geography::Tundra;
        }
    }
    ...
    pub fn seed_plains(&mut self, map: &mut Map, count: u32) {
        ...
        for _ in 0..count {
            ...
            map.tile_at_mut(Location::new(x, y)).geography = Geography::Plains;
        }
    }
    ...
    pub fn grow_continents(&mut self, map: &mut Map, iterations: u32) {
        ...
        let seeds: Vec<Location> = (0..map.height)
            .flat_map(|y| (0..map.width).map(move |x| Location::new(x as u16, y as u16)))
            .filter(|location| map.tile_at(*location).geography == Geography::Plains)
            .collect();
        ...
        for (region, seed) in seeds.iter().enumerate() {
            ...
            for _ in 0..iterations {
                ...
                let geography = self.random_land(next.y as f32, equator);
                map.tile_at_mut(next).geography = geography;
        ...
        let candidates: Vec<(Location, u32)> = Direction::iter()
            .filter_map(|direction| {
                ...
                if !(band_top..band_bottom).contains(&next.y)
                    || !map.tile_at(next).geography.is_water()
    ...
    /// Pick a land geography, favouring plains/grassland. Near the equator
    /// the chance of desert is higher.
    fn random_land(&mut self, latitude: f32, equator: f32) -> Geography {
        ...
        if roll <= desert_chance {
            return Geography::Desert;
        }
        if roll <= desert_chance + 10 {
            return Geography::Mountain;
        }
        if roll <= desert_chance + 25 {
            return Geography::Hills;
        }
        if roll <= desert_chance + 40 {
            return Geography::Forest;
        }
        if roll <= desert_chance + 70 {
            return Geography::Grassland;
        }
        Geography::Plains
    }
}
...
mod tests {
    ...
    fn set_to_ocean_floods_every_tile() {
        ...
        map.tile_at_mut(Location::new(1, 1)).geography = Geography::Plains;
    ...
    fn set_border_tundra_marks_only_the_top_and_bottom_rows() {
        ...
        for x in 0..4 {
            assert_eq!(
                map.tile_at(Location::new(x, 0)).geography,
                Geography::Tundra
            );
            assert_eq!(
                map.tile_at(Location::new(x, 3)).geography,
                Geography::Tundra
            );
        }
        for y in 1..3 {
    ...
    fn seed_plains_drops_the_requested_count_in_the_middle_band() {
        ...
        for y in 0..100 {
            for x in 0..100 {
                ...
                if map.tile_at(location).geography == Geography::Plains {
                    assert!(
        ...
        for seed in seeds {
            let grew = Direction::iter().any(|direction| {
                map.destination(seed, direction)
                    .is_some_and(|neighbour| map.tile_at(neighbour).geography.is_land())
            });
    ...
    fn grow_continents_keeps_each_continent_separate() {
        ...
        for seed in [Location::new(8, 20), Location::new(32, 20)] {
            map.tile_at_mut(seed).geography = Geography::Plains;
        }
    ...
    fn land_components(map: &Map) -> usize {
        ...
        for y in 0..map.height {
            for x in 0..map.width {
                ...
                if map.tile_at(start).geography.is_land() && !visited[y][x] {
                    components += 1;
                    visited[y][x] = true;
                    ...
                    while let Some(location) = stack.pop() {
                        for direction in Direction::iter() {
                            if let Some(neighbour) = map.destination(location, direction) {
                                ...
                                if map.tile_at(neighbour).geography.is_land() && !visited[ny][nx] {
                                    visited[ny][nx] = true;
    ...
    fn grown_land_stays_within_the_middle_band() {
        ...
        for y in 0..40 {
            for x in 0..40 {
                let location = Location::new(x, y);
                if map.tile_at(location).geography.is_land() {
                    assert!(
                        (band_top..band_bottom).contains(&y),
    ...
    fn the_walk_never_overwrites_land_tiles() {
        ...
        generator.seed_plains(&mut map, 4);
        generator.grow_continents(&mut map, 500);
        let before: Vec<(Location, Geography)> = (0..map.height)
            .flat_map(|y| (0..map.width).map(move |x| Location::new(x as u16, y as u16)))
            .filter(|location| map.tile_at(*location).geography.is_land())
            .map(|location| (location, map.tile_at(location).geography))
            .collect();
        generator.grow_continents(&mut map, 10_000);
        for (location, geography) in before {
            assert_eq!(
                map.tile_at(location).geography,
                geography,
    ...
    fn tundra_rows_remain_and_are_not_adjacent_to_land() {
        ...
        for x in 0..map.width {
            ...
            assert_eq!(map.tile_at(location).geography, Geography::Tundra);
            let location = Location::new(x as u16, last as u16);
            assert_eq!(map.tile_at(location).geography, Geography::Tundra);
            assert!(
                !map.tile_at(Location::new(x as u16, 1)).geography.is_land(),
